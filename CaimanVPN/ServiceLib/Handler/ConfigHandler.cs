...
public static class ConfigHandler
{
    ...
    private const string CaimanOnlyServerMessage = "Этот клиент поддерживает только серверы Caiman VPN.";
    private const string CaimanOnlySubscriptionMessage = "Разрешены только подписки Caiman VPN.";
    private const string CaimanCustomConfigMessage = "Пользовательские конфиги разрешены только из подписки Caiman VPN.";
    private static readonly string[] _caimanRoutingModeNames =
    [
        "Белый список (Whitelist)",
        "Черный список (Blacklist)",
        "Глобальный режим (Global)"
    ];
    private static readonly Dictionary<string, string> _caimanRoutingLocalizationMap = new()
    {
    ...
    {
        if (!profileItem.IsSub)
        {
            NoticeManager.Instance.Enqueue(CaimanCustomConfigMessage);
            return -1;
        }
    ...
    public static async Task<int> AddServerCommon(Config config, ProfileItem profileItem, bool toFile = true)
    {
        ...
        if (!CaimanAccessPolicy.IsProfileAllowed(profileItem))
        {
            NoticeManager.Instance.Enqueue(profileItem.ConfigType == EConfigType.Custom
                ? CaimanCustomConfigMessage
                : CaimanOnlyServerMessage);
            return -1;
        }
    ...
    {
        if (!CaimanAccessPolicy.IsSubscriptionUrlAllowed(url))
        {
            NoticeManager.Instance.Enqueue(CaimanOnlySubscriptionMessage);
            return -1;
        }
    ...
    {
        if (!CaimanAccessPolicy.IsSubscriptionUrlAllowed(subItem.Url)
            || !CaimanAccessPolicy.AreSubscriptionUrlsAllowed(subItem.MoreUrl))
        {
            NoticeManager.Instance.Enqueue(CaimanOnlySubscriptionMessage);
            return -1;
        }
    ...
    {
        var caimanWhitelistName = "Белый список (Whitelist)";
        var caimanBlacklistName = "Черный список (Blacklist)";
        var caimanGlobalName = "Глобальный режим (Global)";
        var items = await AppManager.Instance.RoutingItems();
        ...
        if (!blImportAdvancedRules && items.Any(u => IsCaimanRoutingModeRemark(u.Remarks)))
        {
            //migrate
            //TODO Temporary code to be removed later
            ...
            await LocalizeCaimanRoutingItems(items);
            return 0;
        }
    ...
    private static bool IsCaimanRoutingModeRemark(string? text)
    {
        ...
        var normalized = LocalizeCaimanRoutingText(text);
        return _caimanRoutingModeNames.Contains(normalized);
    }

    private static string StripCaimanRoutingVersionPrefix(string text)
    {
        ...
        return _caimanRoutingModeNames.Contains(suffix) ? suffix : text;
    }

    private static async Task LocalizeCaimanRoutingItems(List<RoutingItem> items)
    {
        foreach (var item in items)
        {
            var changed = false;

            var localizedRemarks = LocalizeCaimanRoutingText(item.Remarks);
            ...
            if (rules != null && rules.Count > 0)
            {
                ...
                {
                    var localizedRuleRemarks = LocalizeCaimanRoutingText(rule.Remarks);
    ...
    private static string LocalizeCaimanRoutingText(string? text)
    {
        if (text.IsNullOrEmpty())
        {
        ...
        var localized = text;
        foreach (var (source, target) in _caimanRoutingLocalizationMap)
        {
            localized = localized.Replace(source, target, StringComparison.Ordinal);
        }
        localized = StripCaimanRoutingVersionPrefix(localized);
