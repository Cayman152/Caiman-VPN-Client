...
public class CheckUpdateViewModel : MyReactiveObject
{
    ...
    private readonly string _CaimanVPN = ECoreType.CaimanVPN.ToString();
    ...
    public CheckUpdateViewModel(Func<EViewAction, object?, Task<bool>>? updateView)
    {
        ...
        CheckUpdateCmd.ThrownExceptions.Subscribe(ex =>
        {
            ...
            _ = UpdateView(_CaimanVPN, ex.Message);
        });
    ...
    private void RefreshCheckUpdateItems()
    {
        ...
        {
            CheckUpdateModels.Add(GetCheckUpdateModel(_CaimanVPN));
    ...
    {
        if (coreType == _CaimanVPN && Utils.IsPackagedInstall())
        {
            return new()
            {
                IsSelected = false,
    ...
    private async Task CheckUpdateTask()
    {
        ...
        for (var k = CheckUpdateModels.Count - 1; k >= 0; k--)
        {
            ...
            else if (item.CoreType == _CaimanVPN)
            {
                if (Utils.IsPackagedInstall())
                {
                    await UpdateView(_CaimanVPN, "Not Support");
                    continue;
                }
    ...
    private async Task CheckUpdateN(bool preRelease)
    {
        async Task _updateUI(bool success, string msg)
        {
            await UpdateView(_CaimanVPN, msg);
            if (success)
            {
                await UpdateView(_CaimanVPN, ResUI.OperationSuccess);
                UpdatedPlusPlus(_CaimanVPN, msg);
            }
        }
        await new UpdateService(_config, _updateUI).CheckUpdateGuiN(preRelease)
            .ContinueWith(t => UpdatedPlusPlus(_CaimanVPN, ""));
    }
    ...
    private async Task UpdateFinished()
    {
        if (_lstUpdated.Count > 0 && _lstUpdated.Count(x => x.IsFinished == true) == _lstUpdated.Count)
        {
            ...
            if (_lstUpdated.Any(x => x.CoreType == _CaimanVPN && x.IsFinished == true))
            {
    ...
    private async Task UpgradeN()
    {
        try
        {
            var fileName = _lstUpdated.FirstOrDefault(x => x.CoreType == _CaimanVPN)?.FileName;
            if (fileName.IsNullOrEmpty())
            {
                return;
            }
            ...
            {
                await UpdateView(_CaimanVPN, ResUI.UpgradeAppNotExistTip);
        ...
        {
            await UpdateView(_CaimanVPN, ex.Message);
        }
    }
