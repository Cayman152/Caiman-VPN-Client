...
internal class UpgradeApp
{
    public static void Upgrade(string fileName)
    {
        ...
        try
        {
            var existing = Process.GetProcessesByName(Utils.CaimanVpnExecutableName);
            foreach (var pp in existing)
            {
                var path = pp.MainModule?.FileName ?? "";
                if (path.StartsWith(Utils.GetPath(Utils.CaimanVpnExecutableName)))
                {
                    pp?.Kill();
                    pp?.WaitForExit(1000);
                }
        ...
        Console.WriteLine(Resx.Resource.RestartCaimanVPN);
        Utils.Waiting(2);

        Utils.StartCaimanVpn();
    }
