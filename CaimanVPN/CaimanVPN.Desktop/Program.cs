using CaimanVPN.Desktop.Common;
using System.Runtime.InteropServices;
using System.Text;

namespace CaimanVPN.Desktop;
...
internal class Program
{
    ...
    public static void Main(string[] args)
    {
        try
        {
            if (OnStartup(args, out var startupReason) == false)
            {
                ...
                {
                    ShowStartupDialog("Caiman VPN уже запущен. Проверьте окно приложения или значок в панели задач.", 0x40);
                }
                else
                {
                    var logPath = GetStartupLogPath();
                    ShowStartupDialog($"Не удалось запустить Caiman VPN.{Environment.NewLine}Подробности: {logPath}", 0x10);
                }
    ...
    private static bool OnStartup(string[]? args, out string reason)
    {
        ...
        try
        {
            ...
            {
                _ = new Mutex(true, "CaimanVPN", out var bOnlyOneInstance);
                if (!bOnlyOneInstance)
                {
                    reason = "already-running";
                    return false;
    ...
    private static string GetStartupLogPath()
    {
        ...
        {
            return Path.Combine(Path.GetTempPath(), "CaimanVPN-startup-fatal.log");
        }
    }
